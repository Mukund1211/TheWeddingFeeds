<?php
/** 
 * Author: Shadow Themes
 * Author URL: http://shadow-themes.com
 */

if ( ! defined( 'ABSPATH' ) ) exit;

# Menu Class
if ( ! class_exists( 'Shadow_Menu' ) ) {
	class Shadow_Menu {
        
        public static $custom_fields = array();
        
        function __construct( $custom_fields ) {
            self::$custom_fields = $custom_fields;
            add_filter( 'wp_edit_nav_menu_walker', array( $this, 'edit_nav_menu_walker' ) );
            add_action( 'wp_update_nav_menu_item', array( $this, 'update_nav_menu_item' ), 10, 3 );
        }
        
        function edit_nav_menu_walker( $walker ) {
            $walker = 'Shadow_Menu_Walker_Edit';
            require_once dirname( __FILE__ ) . '/class-menu-walker-edit.php';
            return $walker;
        }
        
        function update_nav_menu_item( $menu_id, $menu_item_id, $args ) {
            $request = stripslashes_deep( $_POST );
            foreach ( self::$custom_fields as $key => $field ) {
                if ( ! isset( $field['sanitize_callback'] ) ) {
                    $field['sanitize_callback'] = 'sanitize_text_field';
                }
                if ( isset( $request[ $key ] ) && isset( $request[ $key ][ $menu_item_id ] ) ) {
                    update_post_meta( $menu_item_id, $key, call_user_func( $field[ 'sanitize_callback' ], $request[ $key ][ $menu_item_id ] ) );
                } else {
                    delete_post_meta( $menu_item_id, $key );
                }
            }
        }
    }
}

# Menu Walker Frontend Class
if ( ! class_exists( 'Shadow_Menu_Walker' ) ) {
    class Shadow_Menu_Walker extends Walker_Nav_Menu {
		public static $custom_fields = array();
		
		function __construct( $custom_fields_in ) {
			self::$custom_fields = $custom_fields_in;
		}
		
        function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            if ( isset( $args->item_spacing ) && 'discard' === $args->item_spacing ) {
                $t = '';
                $n = '';
            } else {
                $t = "\t";
                $n = "\n";
            }
            $indent = ( $depth ) ? str_repeat( $t, $depth ) : '';

            $classes = empty( $item->classes ) ? array() : (array) $item->classes;
            $classes[] = 'menu-item-' . $item->ID;
            
            $args = apply_filters( 'nav_menu_item_args', $args, $item, $depth );
            
			$class_names = join( ' ', apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item, $args, $depth ) );
            
            if ( $depth == 0 ) {
                foreach ( self::$custom_fields as $key => $field ) {
                    $menu_item_id = $item->ID;
                    $option_value = get_post_meta( $menu_item_id, $key );
                    if ( is_array( $option_value ) && !empty( $option_value ) ) {
                        $class_names .= ' ' . $option_value[ 0 ];
                    }
                }
            }
            
			$class_names = $class_names ? ' class="' . esc_attr( $class_names ) . '"' : '';
            
			$id = apply_filters( 'nav_menu_item_id', 'menu-item-'. $item->ID, $item, $args, $depth );
			$id = $id ? ' id="' . esc_attr( $id ) . '"' : '';
	 
			$output .= $indent . '<li' . $id . $class_names .'>';
	 
			$atts = array();
			$atts['title']  = ! empty( $item->attr_title ) ? $item->attr_title : '';
			$atts['target'] = ! empty( $item->target )     ? $item->target     : '';
			$atts['rel']    = ! empty( $item->xfn )        ? $item->xfn        : '';
			$atts['href']   = ! empty( $item->url )        ? $item->url        : '';

            $atts = apply_filters( 'nav_menu_link_attributes', $atts, $item, $args, $depth );            

			$attributes = '';
			foreach ( $atts as $attr => $value ) {
				if ( ! empty( $value ) ) {
					$value = ( 'href' === $attr ) ? esc_url( $value ) : esc_attr( $value );
					$attributes .= ' ' . $attr . '="' . $value . '"';
				}
			}
            
			$title = apply_filters( 'the_title', $item->title, $item->ID );

			$title = apply_filters( 'nav_menu_item_title', $title, $item, $args, $depth );
	 
			$item_output = $args->before;
			$item_output .= '<a'. $attributes .'>';
			$item_output .= $args->link_before . $title . $args->link_after;
			$item_output .= '</a>';
			$item_output .= $args->after;

			$output .= apply_filters( 'walker_nav_menu_start_el', $item_output, $item, $depth, $args );
        }
    }
}

?>