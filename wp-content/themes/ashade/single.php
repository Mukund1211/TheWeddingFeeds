...
if ( ! post_password_required() ) {
	...
	if ( true == $title_state && 'vertical' == $layout ) {
	...
	?>
		<main id="post-<?php the_ID(); ?>" <?php post_class( 'ashade-content-wrap' ); ?>>
			<div class="ashade-content-scroll">
				<?php
					if ( true == $title_state && 'horizontal' == $layout ) {
						get_template_part( 'template-parts/page-title' );
					}
				?>
				<div class="ashade-content">
					<div class="ashade-row <?php echo ( 'left' == $ashade_sidebar ? 'is-reverse' : '' ); ?>">
						<div class="ashade-col col-<?php echo ( 'none' == $ashade_sidebar ? '12' : '9' ); ?>">
							<!-- Post Featured Image -->
							<?php
							if ( Ashade_Core::get_prefer( 'ashade-post-image' ) == true && Ashade_Core::get_fimage_url() ) {
								$featured_image_url = Ashade_Core::get_fimage_url();
								if ( class_exists( 'Aq_Resize' ) ) {
									echo '
									<div class="ashade-post-featured-image">
										<img src="'. ( aq_resize( esc_url( $featured_image_url ), '1920', '', true, true, false ) ? aq_resize( esc_url( $featured_image_url ), '1920', '' ) : esc_url($featured_image_url ) ) . '" alt="' . esc_attr( get_the_title() ) . '">
									</div>
									';
								} else {
									echo '
							...
							if ( Ashade_Core::get_prefer( 'ashade-post-comments' ) == true ) {
								comments_template();
							}
							?>
						</div><!-- .ashade-col -->
				...
				<?php 
				if ( true == $back_state && 'horizontal' == $layout ) {
					get_template_part( 'template-parts/back-to-top' );
				}
				if ( true == $back_state && 'vertical' == $layout ) {
