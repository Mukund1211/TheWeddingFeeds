...
jQuery(window).on('load', function() {
	// Page Subtitle
	if (jQuery('input[name="ashade-page-subtitle"]').length) {
		let $original_input = jQuery('input[name="ashade-page-subtitle"]'),
			$ashade_subtitle = jQuery('<input type="text" placeholder="'+ $original_input.attr('placeholder') +'" class="ashade-subtitle-field">');
		$ashade_subtitle.val($original_input.val());
		
		if (jQuery('body').find('.wp-block.editor-post-title__block').length) {
			jQuery('body').find('.editor-post-title').addClass('has-overhead');
			jQuery('body').find('.wp-block.editor-post-title__block').prepend($ashade_subtitle);
		} else if (jQuery('#titlewrap').length) {
			$ashade_subtitle.addClass('is-default-editor');
			jQuery('#titlewrap').prepend($ashade_subtitle);
		}
		
		$ashade_subtitle.on('keyup', function(e) {
			$original_input.val($ashade_subtitle.val());
		}).on('change', function() {
			$original_input.val($ashade_subtitle.val());
		});
	}
